    ...
    for row_number in range(number_of_rows):
        ...
        try:
            ...
            try:
                ...
                if results is not None:
                    ...
                    if "age" in new_product_price:
                        ...
                        try:
                            ...
                            html = driver.page_source
                            soup = BeautifulSoup(html, features="lxml")
                            results = soup.find("span", id="aod-price-1")
                            if results is not None:
                                price_text = results.find("span", class_="a-offscreen").get_text()
                                price_without_sign = price_text[1:]
                                new_product_price = price_without_sign
                                ...
                                # New Delivery Price for only this scenario
                                find_delivery_price = False
                                results1 = soup.find("div",
                                                     class_="a-section a-spacing-none a-padding-base aod-information-block aod-clear-float")
                                results2 = results1.find("div",
                                                     id="mir-layout-DELIVERY_BLOCK-slot-PRIMARY_DELIVERY_MESSAGE_LARGE")
                                results3 = results2.find("span", attrs={'data-csa-c-delivery-price': True})
                                print("New delivery price: " + results3["data-csa-c-delivery-price"])
                                if (results3["data-csa-c-delivery-price"] == "FREE"):
                                    new_delivery_price = 0
                                else:
                                    try:
                                        if "£" in results3["data-csa-c-delivery-price"]:
                                            new_delivery_price = \
                                            re.findall("\d+\.\d+", results3["data-csa-c-delivery-price"])[0]
                                            new_delivery_price = float(new_delivery_price)
                                        else:
                                            new_delivery_price = float(results3["data-csa-c-delivery-price"])
                                    except:
                ...
                # New Product Price wasn't findable.
                print("New product price wasn't findable")
                print(e)
